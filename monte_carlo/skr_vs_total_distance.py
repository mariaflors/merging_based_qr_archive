...
def mb_run_it(total_distance, k, p, dephasing_time, growth, patch, num_samples_per_point, cs=2e8):
    ...
    return [total_distance, wt_eb, std_wt_eb, ez_eb, std_ez_eb, ex_eb, std_ex_eb]
...
def sb_run_it(total_distance, k, p, dephasing_time, num_samples_per_point, cs=2e8):
    ...
    return [total_distance, wt_sb, std_wt_sb, ez_sb, std_ez_sb, ex_sb, std_ex_sb]
    ...
    for dephasing_time in dephasing_times:
        for k in ks:
            for p in ps:
                ...
                np.save(f"results/skr_td_sb_k={k}_p={p}_T={dephasing_time}_td={total_distances[-1]}",
                        (td_sb, wt_sb, std_wt_sb, ez_sb, std_ez_sb, ex_sb, std_ex_sb))
                for growth in growths:
                    for patch in patches:
                        ...
                        np.save(
                            f"results/skr_td_mb_k={k}_p={p}_T={dephasing_time}_g={growth}_plim={patch}_td={total_distances[-1]}",
                            (td_eb, wt_eb, std_wt_eb, ez_eb, std_ez_eb, ex_eb, std_ex_eb))
    return
